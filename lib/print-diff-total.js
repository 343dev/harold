...
export default function printDiffTotal({ left, right, leftCaption, rightCaption } = {}) {
	...
	const categories = Object.keys(left)
		.filter(category => Object.keys(right).includes(category) && !/all|other/.test(category));

	for (const category of categories) {
		table.push(generateTableRow(category, left[category], right[category]));
	}
